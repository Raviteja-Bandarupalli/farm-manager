...
import { Plus, Trash2, Edit, Utensils, Wheat, Beef, Droplet, FlaskConical } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function FeedLogsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isDistributionValid) {
      alert("Distribution sum or ingredient sum must match Total Batch Weight.")
      return
    }

    try {
      const numericDistribution: Record<string, number> = {}
      Object.entries(formData.distribution).forEach(([hid, kg]) => {
        if (Number(kg) > 0) numericDistribution[hid] = Number(kg)
      })
          ...
          <DialogContent className="max-w-2xl max-h-[95vh] overflow-y-auto p-0 border-none shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6 bg-white">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setFormData({...formData, farmId: v, distribution: {}})}
                    required
                  >
                ...
                {[
                  ...
                  <div key={item.id} className="p-2 border rounded-md space-y-1 bg-slate-50">
                    ...
                    <Input
                      type="number"
                      step="0.1"
                      className="h-8 font-black text-xs"
                    ...
                    {farmHouses.map(house => (
                      <div key={house.id} className="flex items-center justify-between p-2 border rounded hover:bg-slate-50 transition-colors">
                        ...
                        <div className="flex items-center gap-2">
                          <Input
                            type="number"
                            className="h-8 w-24 font-black text-right"
                            placeholder="0"
                            value={formData.distribution[house.id] || ""}
                            onChange={(e) => setFormData({
                              ...formData,
                              distribution: {...formData.distribution, [house.id]: e.target.value}
                            })}
                          />
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                ...
                <Button
                  type="submit"
                  disabled={!isDistributionValid || !formData.farmId}
                  className="h-10 bg-slate-900 text-white hover:bg-slate-800 text-[11px] font-black uppercase tracking-widest px-12"
                >
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">
                          Dist: {Object.keys(log.distribution).length} Houses
                        </span>
                      </div>
                    </TableCell>
